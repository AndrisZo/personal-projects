...
class Minesweep extends World{
  ...
  int bombsRemaining;
  int unrevealedRemaining;
  boolean started;
  
  // Maximum percentage of bombs allowed
  static final double MAX_BOMBS = 0.3;
  static final Color LOSE_MESSAGE_COLOR = Color.RED;
  static final Color WIN_MESSAGE_COLOR = Color.BLUE;
  ...
  public WorldScene makeScene() {
    ...
    if (this.playerBlewUp) {
      board.placeImageXY(this.explodedMessage(this.height * Cell.CELL_SIZE / 5), 
          this.width * Cell.CELL_SIZE / 2, this.height * Cell.CELL_SIZE / 2);
    } else if (this.bombsRemaining == 0) {
      board.placeImageXY(this.winMessage(this.height * Cell.CELL_SIZE / 5), 
          this.width * Cell.CELL_SIZE / 2, this.height * Cell.CELL_SIZE / 2);
    }
    return board;
  }
  ...
  private WorldImage explodedMessage(double fontSize) {
    return new TextImage("Boom!", fontSize, LOSE_MESSAGE_COLOR);
  }
  
  private WorldImage winMessage(double fontSize) {
    return new TextImage("You win!", fontSize, WIN_MESSAGE_COLOR);
  }
  ...
  public void onMousePressed(Posn loc, String button) {
    if (!this.playerBlewUp && this.bombsRemaining > 0) {
...
class Cell {
  ...
  static final Color MARKED = Color.GREEN;
  ...
  // Did the user reveal any bombs by clearing all of this square's neighbors?
  // EFFECT: If the user has already marked all of the bombs around this cell,
  // Reveal any other cells
  private boolean clearNeighbors() {
    boolean exploded = false;
    if (this.markCount() == this.neighborBombs) {
      for (Cell c : this.neighbors) {
        exploded = exploded || c.clear();
      }
      return exploded;
    } else {
      return false;
  ...
  // Did the user reveal any bombs by clearing this tile?
  // EFFECT: Clear this tile. If it has no neighboring bombs, reveal its neighbors.
  ...
  // How many of this cell's neighbors are marked?
